...
from featransform.core.models import TransformationResult, FeatureInfo
from featransform.core.constants import (
    DEFAULT_N_COMPONENTS, COMPONENT_PREFIX,
...
class DimensionalityStrategy(BaseFeatureEngineer):
    ...
    def _initialize_models(self, X: pd.DataFrame) -> None:
        ...
        if isinstance(self.n_components, int) and self.n_components > max_components:
            self._log(f"Requested {self.n_components} components exceeds max {max_components}, using default")
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'DimensionalityStrategy':
        ...
        self._feature_names = self._generate_feature_names(
            int(actual_components),  # Ensure integer
            COMPONENT_PREFIX
        )
        return self
