...
from featransform.core.enums import SelectionStrategy, OptimizationMetric, TaskType
from featransform.optimization.evaluator import BaseEvaluator, CatBoostImportance, XGBoostImportance, ImportanceCalculator
...
class FeatureSelector(BaseSelector, BaseEvaluator):
    ...
    def optimize(self, X_train: pd.DataFrame, y_train: pd.Series, 
                 ...
                 thresholds: List[float]) -> Tuple[List[str], float, List[Dict]]:
        if not self._fitted:
    ...
    def select_by_threshold(self, threshold: float) -> List[str]:
        ...
        return selected if len(selected) >= self.min_features else \
               self._importance_df.head(self.min_features)['Feature Id'].tolist()
    ...
    def get_feature_scores(self) -> Dict[str, float]:
        ...
        if self._importance_df is not None:
            return dict(zip(
                self._importance_df['Feature Id'], 
                self._importance_df['Importances']
            ))
        return {}
    ...
    def _init_calculators(self, use_catboost: bool, use_xgboost: bool, n_estimators: int) -> List[ImportanceCalculator]:
        calculators = []
    ...
    def _compute_importance(self, X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
        ...
        df = pd.concat(importances).groupby('Feature Id')['Importances'].mean().reset_index() \
             if len(importances) > 1 else importances[0]
