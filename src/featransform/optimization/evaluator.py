...
from featransform.core.enums import TaskType, OptimizationMetric
from featransform.core.constants import DEFAULT_RANDOM_STATE, DEFAULT_N_JOBS
...
class CatBoostImportance(ImportanceCalculator):
    ...
    def calculate(self, X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
        ...
        params = {'n_estimators': self.n_estimators, 'random_state': self.random_state, 'verbose': False}
...
class BaseEvaluator(ABC):
    ...
    def evaluate_subset(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        y_proba = self._eval_model.predict_proba(X_val) if self._task_type != TaskType.REGRESSION else None
    ...
    def _init_eval_model(self):
        params = {
            ...
            'random_state': DEFAULT_RANDOM_STATE, 'n_jobs': DEFAULT_N_JOBS, 'verbosity': 0
        }
        if self._task_type == TaskType.REGRESSION:
            self._eval_model = xgb.XGBRegressor(**params)
        else:
            self._eval_model = xgb.XGBClassifier(**params, use_label_encoder=False, eval_metric='logloss')
    
    def _calculate_metric(self, y_true: pd.Series, y_pred: np.ndarray, y_proba: np.ndarray = None) -> float:
        if self.metric is None:
            self.metric = OptimizationMetric.MSE if self._task_type == TaskType.REGRESSION else OptimizationMetric.F1
        ...
        metrics = {
            ...
            OptimizationMetric.PRECISION: lambda: self._classification_metric(precision_score, y_true, y_pred),
            OptimizationMetric.RECALL: lambda: self._classification_metric(recall_score, y_true, y_pred),
